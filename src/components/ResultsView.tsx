      ...
      <div className="w-full max-w-2xl bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
        ...
        <div className="space-y-4 mb-6">
          <div className="flex justify-between items-center p-3 bg-gray-50 dark:bg-gray-700 rounded-md">
            <span className="text-gray-700 dark:text-gray-200">Execution Time:</span>
            <span className="font-mono text-gray-700 dark:text-gray-200">{results.executionTime}</span>
          </div>
          <div className="flex justify-between items-center p-3 bg-gray-50 dark:bg-gray-700 rounded-md">
            <span className="text-gray-700 dark:text-gray-200">Memory Used:</span>
            <span className="font-mono text-gray-700 dark:text-gray-200">{results.memoryUsed}</span>
          </div>

          {/* Only show detailed test cases if results did not pass overall */}
          {!results.passed && results.testCases && results.testCases.length > 0 && (
            <>
              <h3 className="font-medium text-lg mt-6 mb-2 text-neutral-750 dark:text-white">Test Results:</h3>
              {results.testCases.map((test, index) => (
                <div key={index} className={`p-3 rounded-md flex items-start ${test.passed ? 'bg-green-50 dark:bg-green-900/20' : 'bg-red-50 dark:bg-red-900/20'}`}>
                  ...
                  <div>
                    <p className="font-mono text-sm text-gray-700 dark:text-gray-200">Input: {test.input}</p>
                    <p className="font-mono text-sm text-gray-700 dark:text-gray-200">Expected: {test.output}</p>
        ...
        <div className="flex flex-col sm:flex-row justify-center items-center space-y-3 sm:space-y-0 sm:space-x-3">
          ...
          <button 
            onClick={onTryAgain} 
            className="flex items-center justify-center w-full sm:w-auto px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 transition-colors"
          >
            Try Another Problem
          </button>
        </div>
