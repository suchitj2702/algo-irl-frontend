...
export function LoadingSequence({
 ...
 useEffect(() => {
  ...
  // Use company name directly (already formatted from AppRouter)
 ...
 return <div className="flex flex-col items-center justify-center min-h-[calc(100vh-3.5rem)] p-4 bg-surface dark:bg-surface">
   <div className="w-full max-w-2xl">
    <div className="mb-8 relative">
     <div className="h-2 bg-slate-200 dark:bg-slate-700 rounded-full overflow-hidden">
      <div className="h-full bg-gradient-to-r from-mint-500 to-mint-600 rounded-full transition-all duration-700 ease-in-out" style={{
      width: `${progress}%`
     }}></div>
     </div>
    ...
    <div className="space-y-6">
     ...
     return <div key={index} className={`transition-all duration-500 flex items-start ${isStepCompleted ? 'text-content' : isCurrentStep ? 'animate-pulse text-content-muted dark:text-content-subtle' : 'text-content-subtle dark:text-content-muted'}`}>
        <div className={`mr-3 p-1 rounded-full ${isStepCompleted ? 'bg-mint-500/20 text-teal-600 dark:text-teal-400' : isCurrentStep ? 'bg-mint-500/20 text-teal-600 dark:text-teal-400 animate-pulse' : 'bg-slate-200 dark:bg-slate-700'}`}>
    ...
    {!isCompleted && (
     <div className="mt-12 flex justify-center">
      <div className="relative w-16 h-16">
       ...
       <div className="absolute inset-0 rounded-full border-4 border-t-transparent border-r-mint-400 border-b-transparent border-l-transparent animate-spin" style={{
       animationDuration: '1.5s'
      }}></div>
       <div className="absolute inset-0 rounded-full border-4 border-t-transparent border-r-transparent border-b-teal-500 border-l-transparent animate-spin" style={{
